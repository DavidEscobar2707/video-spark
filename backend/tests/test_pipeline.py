...
@pytest.mark.asyncio
async def test_run_image_story_pipeline_generates_story_segments(monkeypatch, tmp_path):
    states: list[tuple[str, int]] = []
    ...
    async def fake_generate_script(payload):
        return payload.source.prompt

    async def fake_split_image_story_scenes(script, payload):
        _ = payload
        return [
            {"text": f"{script} beat {index}", "visualPrompt": f"frame {index}", "durationSeconds": 5, "storyBible": "hero bible"}
    ...
    async def fake_build_captions(voice_artifact, captions_config, runtime_config):
        ...
        _ = captions_config
        _ = runtime_config
        ass_path = tmp_path / "story-captions.ass"
        srt_path = tmp_path / "story-captions.srt"
        ass_path.write_text("ass", encoding="utf-8")
        srt_path.write_text("srt", encoding="utf-8")
        return {"assPath": ass_path.as_posix(), "srtPath": srt_path.as_posix()}

    async def fake_generate_story_segments(scenes, config):
        _ = config
        for index, _scene in enumerate(scenes):
            yield {
                "sceneIndex": index,
                "clipPath": (tmp_path / f"story-clip-{index}.mp4").as_posix(),
                "imagePath": (tmp_path / f"story-image-{index}.png").as_posix(),
                "operationName": f"image-{index}",
    ...
    async def fake_compose_video(scenes, media_urls, voice_url, subtitle_ass_path, config):
        ...
        _ = voice_url
        _ = subtitle_ass_path
        final_path = tmp_path / config["jobId"] / "image-story-final.mp4"
        final_path.parent.mkdir(parents=True, exist_ok=True)
        final_path.write_bytes(b"mp4")
        return final_path.as_posix()
    ...
    def fake_upload_final_video(local_path, runtime_config):
        _ = local_path
        return f"https://storage.example.com/{runtime_config['jobId']}/image-story-final.mp4"

    monkeypatch.setattr(pipeline, "update_job_state", fake_update_job_state)
    monkeypatch.setattr(pipeline, "mark_job_completed", fake_mark_job_completed)
    monkeypatch.setattr(pipeline, "generate_script", fake_generate_script)
    monkeypatch.setattr(pipeline, "split_image_story_scenes", fake_split_image_story_scenes)
    monkeypatch.setattr(pipeline, "generate_voice", fake_generate_voice)
    monkeypatch.setattr(pipeline, "build_captions", fake_build_captions)
    monkeypatch.setattr(pipeline, "generate_story_segments", fake_generate_story_segments)
    monkeypatch.setattr(pipeline, "compose_video", fake_compose_video)
    monkeypatch.setattr(pipeline, "upload_final_video", fake_upload_final_video)

    job = {
        "job_id": "story-job-xyz",
        "project_id": "story-project-xyz",
        "tenant_id": "tenant-xyz",
        "config": {
            "workflow": "image-story-to-video",
            "source": {"prompt": "Tell a product story across five scenes."},
            "media": {"type": "ai-video"},
            "voice": {"enabled": True, "voiceId": "voice-123"},
    ...
    assert result["status"] == "completed"
    assert result["output_video_url"] == "https://storage.example.com/story-job-xyz/image-story-final.mp4"
    assert result["pipeline_state"]["stage"] == "completed"
    assert Path(result["pipeline_state"]["finalLocalPath"]).exists()
    assert result["pipeline_state"]["subtitleAssPath"].endswith("story-captions.ass")
    assert result["pipeline_state"]["storyBible"] == "hero bible"
    assert [stage for stage, _progress in states] == [
        "generating_script",
        "script_ready",
        "generating_voice",
...
@pytest.mark.asyncio
async def test_process_next_job_writes_non_empty_error_message(monkeypatch):
    recorded: dict[str, object] = {}

    async def fake_claim_next_job():
        return {"job_id": "job-err", "project_id": "project-err", "tenant_id": "tenant-err", "config": {}}

    async def fake_run_pipeline(job):
        _ = job
        raise RuntimeError()

    async def fake_mark_job_failed(job, error_message):
        recorded["job"] = job
        recorded["error_message"] = error_message
        return {**job, "status": "failed"}

    monkeypatch.setattr(pipeline, "claim_next_job", fake_claim_next_job)
    monkeypatch.setattr(pipeline, "_run_pipeline", fake_run_pipeline)
    monkeypatch.setattr(pipeline, "mark_job_failed", fake_mark_job_failed)

    handled = await pipeline.process_next_job()

    assert handled is True
    assert recorded["error_message"]
    assert "RuntimeError" in str(recorded["error_message"])
