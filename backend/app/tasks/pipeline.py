...
from app.tasks.steps.image_story import generate_story_segments
from app.tasks.steps.media import generate_scene_clips
from app.tasks.steps.script import generate_script, split_avatar_scenes, split_image_story_scenes, split_scenes
...
async def _run_pipeline(job: dict[str, object]) -> dict[str, object]:
    ...
    if payload.workflow == Workflow.IMAGE_STORY_TO_VIDEO:
        return await _run_image_story_pipeline(job, payload, runtime_config)
...
async def _run_image_story_pipeline(
    job: dict[str, object],
    payload: RenderRequest,
    runtime_config: dict[str, object],
) -> dict[str, object]:
    job = await update_job_state(job, stage="generating_script", progress=15)
    script = await generate_script(payload)
    scenes = await split_image_story_scenes(script, payload)
    job = await update_job_state(
        job,
        stage="script_ready",
        progress=30,
        script_text=script,
        scenes=scenes,
        pipeline_extra={"storyBible": scenes[0].get("storyBible") if scenes else None},
    )
    ...
    job = await update_job_state(job, stage="generating_images", progress=70)
    media_urls: list[str] = []
    scene_operations: list[dict[str, object]] = []
    total_scenes = max(len(scenes), 1)
    async for segment in generate_story_segments(scenes, runtime_config):
        media_urls.append(str(segment["clipPath"]))
        scene_operations.append(
            {
...
async def process_next_job() -> bool:
    ...
    try:
        ...
        await mark_job_failed(job, message)
    return True
