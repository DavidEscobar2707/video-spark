...
class Workflow(str, Enum):
    ...
    IMAGE_STORY_TO_VIDEO = "image-story-to-video"
...
class ImageStoryRenderRequest(BaseModel):
    model_config = ConfigDict(extra="allow", populate_by_name=True)
    ...
    @model_validator(mode="after")
    def validate_inputs(self) -> "ImageStoryRenderRequest":
        if not (self.source.text or self.source.prompt):
            raise ValueError("source.text or source.prompt is required for image-story-render.")
        return self
...
class ScriptGenerationRequest(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_inputs(self) -> "ScriptGenerationRequest":
        if not self.prompt.strip():
            raise ValueError("prompt is required for generate-script.")
        return self
...
class ScriptGenerationResponse(BaseModel):
    ...
    hook: str
    script: str
    scene_beats: list[ScriptBeat] = Field(alias="sceneBeats")
...
class VoiceOption(BaseModel):
    ...
    preview_audio_url: str | None = Field(default=None, alias="previewAudioUrl")
...
class CaptionPresetOption(BaseModel):
    id: str
    name: str
    description: str
...
class CaptionPresetListResponse(BaseModel):
    presets: list[CaptionPresetOption]
